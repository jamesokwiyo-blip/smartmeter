...
const Dashboard = () => {
  ...
  const loadPurchaseHistory = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setPurchases(list);
        if (list.length > 0 && !diagnosticMeterNumber) {
          setDiagnosticMeterNumber(list[0].meterNumber || "");
        }
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    if (!diagnosticMeterNumber && purchases.length > 0) {
      setDiagnosticMeterNumber(purchases[0].meterNumber || "");
    }
  }, [purchases]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await energyAPI.getByMeter(meter);
        if (res.data?.success && res.data?.data) setEnergyData(res.data.data);
        else setEnergyData(null);
      } catch {
        setEnergyData(null);
      }
    };
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* Smart Meter Diagnostics - live PZEM values from device */}
            <div>
                ...
                {energyData ? (
                  <>
                    <div className="space-y-3">
                      ...
                      {typeof energyData.voltage === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Voltage</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.voltage.toFixed(2)} V</span>
                        </div>
                      )}
                      {typeof energyData.current === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Current</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.current.toFixed(2)} A</span>
                        </div>
                      )}
                      {typeof energyData.power === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Power</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.power.toFixed(1)} W</span>
                        </div>
                      )}
                      {typeof energyData.totalEnergy === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Total Energy (sensor)</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.totalEnergy.toFixed(2)} kWh</span>
                        </div>
                      )}
                      {typeof energyData.consumedKwh === "number" && (
                        <div className="flex items-center justify-between p-3 bg-amber-50 border border-amber-200 rounded-lg">
                          <span className="text-sm font-medium text-amber-800">Consumed (session)</span>
                          <span className="text-xs font-bold text-amber-700">{energyData.consumedKwh.toFixed(2)} kWh</span>
                        </div>
                      )}
                      {typeof energyData.remainingKwh === "number" && (
                        <div className="flex items-center justify-between p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                          <span className="text-sm font-medium text-emerald-800">Remaining</span>
                          <span className="text-xs font-bold text-emerald-700">{energyData.remainingKwh.toFixed(2)} kWh</span>
                        </div>
                      )}
                      {typeof energyData.frequency === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Frequency</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.frequency.toFixed(2)} Hz</span>
                        </div>
                      )}
                      {typeof energyData.powerFactor === "number" && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                          <span className="text-sm font-medium text-slate-800">Power Factor</span>
                          <span className="text-xs font-bold text-slate-700">{energyData.powerFactor.toFixed(2)}</span>
                        </div>
                      )}
                    </div>
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-xs text-blue-700 text-center">
                        Meter: {energyData.meterNumber || "—"} · Last update: {energyData.timestampFormatted || energyData.serverTimestamp || "—"}
                      </p>
                    </div>
                  </>
                ) : (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                      <span className="text-sm font-medium text-slate-600">No live data</span>
                      <span className="text-xs text-slate-500">Enter meter &amp; ensure device is sending</span>
                    </div>
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-xs text-blue-700 text-center">
                        Diagnostics show when the meter has sent data (meter: {diagnosticMeterNumber || "—"})
                      </p>
                    </div>
                  </div>
                )}
